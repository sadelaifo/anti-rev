...
/* ------------------------------------------------------------------ */
/*  Symbol-completion closure                                          */
/*                                                                      */
/*  DT_NEEDED alone misses *implicit* inter-lib symbol dependencies:    */
/*  a business lib that references a symbol exported by a sibling       */
/*  encrypted lib without any DT_NEEDED edge between them (a common     */
/*  "the host process already has it in global scope" build pattern).   */
/*  Under the dlopen_shim per-dep preload loop each encrypted lib is    */
/*  dlopen'd in isolation, so the implicit provider isn't in scope and  */
/*  the load fails with "undefined symbol ... (fatal)".                 */
/*                                                                      */
/*  Fix: after the DT_NEEDED graph is built, for every encrypted lib    */
/*  resolve its undefined symbols against the *other* encrypted libs'   */
/*  exported symbols and add the resulting consumer->provider edges to  */
/*  the same adjacency the closure walk uses.  The provider then rides  */
/*  along in OP_GET_CLOSURE and gets preloaded with the rest — no       */
/*  __r_NP needed, and preload (hence protobuf ODR dedup) stays on.     */
/* ------------------------------------------------------------------ */

/* Tiny open-addressing string->int map (FNV-1a + linear probe).  Used
 * once at daemon startup, then torn down.  value convention:
 *   -1  : symbol is referenced (undefined) somewhere, no encrypted
 *         provider found yet
 *   >=0 : index of the first encrypted lib that defines it */
typedef struct {
    char  **keys;   /* strdup'd; NULL == empty slot */
    int    *vals;
    size_t  cap;    /* power of two */
    size_t  len;
} symmap_t;

static uint64_t symmap_hash(const char *s) {
...
static int symmap_init(symmap_t *m, size_t want) {
    m->cap = 64;
    while (m->cap < want)
        m->cap <<= 1;
...
static int symmap_grow(symmap_t *m) {
    size_t ncap = m->cap << 1;
    char **nk = calloc(ncap, sizeof(char *));
    int   *nv = malloc(ncap * sizeof(int));
    if (!nk || !nv) {
        free(nk);
        free(nv);
        return -1;
    ...
    for (size_t i = 0; i < m->cap; i++) {
        if (!m->keys[i])
            continue;
        size_t j = symmap_hash(m->keys[i]) & nmask;
        while (nk[j])
            j = (j + 1) & nmask;
        nk[j] = m->keys[i];
        nv[j] = m->vals[i];
...
/* Insert key with `val` only if absent (first writer wins). */
static void symmap_put_if_absent(symmap_t *m, const char *key, int val) {
    if ((m->len + 1) * 10 >= m->cap * 7 && symmap_grow(m) < 0)
        return; /* OOM — drop silently, degrades to no edge for this sym */
    size_t mask = m->cap - 1;
    size_t i = symmap_hash(key) & mask;
    while (m->keys[i]) {
...
/* Return pointer to the stored value for `key`, or NULL if absent. */
static int *symmap_find(symmap_t *m, const char *key) {
    if (m->cap == 0)
        return NULL;
    size_t mask = m->cap - 1;
    size_t i = symmap_hash(key) & mask;
...
/* mmap an fd read-only.  Returns base (and *out_size) or NULL. */
static const uint8_t *map_lib_ro(int fd, size_t *out_size) {
    struct stat st;
    if (fstat(fd, &st) < 0 || st.st_size < (off_t) sizeof(Elf64_Ehdr))
        return NULL;
...
/* Iterate .dynsym of a mapped ELF64 image via section headers.  For
 * every named STB_GLOBAL/STB_WEAK symbol, invokes
 * cb(name, defined, weak, user).  Returns 0 on success, -1 if the
 * image has no usable section headers / .dynsym — graceful: that lib
 * simply contributes no symbol-completion edges (current behaviour).
 *
 * Section headers (not PT_DYNAMIC hash walking) on purpose: the daemon
 * decrypts the full original .so, whose section table is intact for
 * every real-world shared object, and this keeps the parser small. */
static int elf64_foreach_dynsym(const uint8_t *base, size_t size,
                                void (*cb)(const char *, int, int, void *),
                                void *user)
{
    const Elf64_Ehdr *eh = (const Elf64_Ehdr *) base;
    if (elf64_header_ok(eh, size) < 0)
    ...
    if (eh->e_shentsize != sizeof(Elf64_Shdr))
        return -1;
    if (eh->e_shoff + (uint64_t) eh->e_shnum * sizeof(Elf64_Shdr) > size)
        return -1;
    ...
    if (dynsym->sh_link >= eh->e_shnum)
        return -1;
    const Elf64_Shdr *strsec = &sh[dynsym->sh_link];
    if (dynsym->sh_offset + dynsym->sh_size > size)
        return -1;
    if (strsec->sh_offset + strsec->sh_size > size)
        return -1;

    const Elf64_Sym *syms = (const Elf64_Sym *) (base + dynsym->sh_offset);
    size_t nsym = dynsym->sh_size / sizeof(Elf64_Sym);
    const char *str = (const char *) (base + strsec->sh_offset);
    size_t strmax = strsec->sh_size;

    for (size_t i = 0; i < nsym; i++) {
        uint32_t no = syms[i].st_name;
        if (no == 0 || no >= strmax)
            continue;
        unsigned b = ELF64_ST_BIND(syms[i].st_info);
        if (b != STB_GLOBAL && b != STB_WEAK)
            continue;
        int defined = (syms[i].st_shndx != SHN_UNDEF);
        int weak = (b == STB_WEAK);
        cb(str + no, defined, weak, user);
    }
    return 0;
}

/* Pass A — every STB_GLOBAL *undefined* symbol becomes a "wanted"
 * entry (val=-1).  Weak-undefined is skipped: the linker lets it
 * resolve to 0, so it isn't a hard dependency and adding an edge for
 * it would pull libs in unnecessarily. */
static void symc_collect_wanted(const char *name, int defined, int weak,
                                void *u) {
    if (defined || weak)
        return;
    symmap_put_if_absent((symmap_t *) u, name, -1);
}

struct symc_provider_ctx { symmap_t *m; int lib; };

/* Pass B — first encrypted lib that *defines* a wanted symbol claims
 * it (mirrors ELF first-definition-wins interposition). */
static void symc_record_provider(const char *name, int defined, int weak,
                                 void *u) {
    (void) weak;
    if (!defined)
        return;
    struct symc_provider_ctx *c = (struct symc_provider_ctx *) u;
    int *v = symmap_find(c->m, name);
    if (v && *v == -1)
        *v = c->lib;
}

struct symc_edge_ctx { symmap_t *m; int lib; int added; };

/* Pass C — for each STB_GLOBAL undefined symbol with an encrypted
 * provider, add a consumer->provider edge (deduped, bounded).  A
 * provider that's already a DT_NEEDED dep just no-ops here; the
 * closure walk dedups via its visited[] set regardless. */
static void symc_add_edge(const char *name, int defined, int weak,
                          void *u) {
    if (defined || weak)
        return;
    struct symc_edge_ctx *c = (struct symc_edge_ctx *) u;
    int *v = symmap_find(c->m, name);
    if (!v)
        return;
    int j = *v;
...
/* Wire implicit inter-lib symbol dependencies into the closure graph.
 * Returns the number of edges added, or -1 on allocation failure. */
static int augment_closure_with_symbols(const int *lib_fds,
                                        const char (*lib_names)[MAX_NAME + 1],
                                        int nlibs)
{
    (void) lib_names;
    symmap_t m;
    if (symmap_init(&m, 4096) < 0)
        return -1;

    for (int i = 0; i < nlibs; i++) {
        size_t sz;
        const uint8_t *p = map_lib_ro(lib_fds[i], &sz);
        if (!p)
            continue;
        elf64_foreach_dynsym(p, sz, symc_collect_wanted, &m);
        munmap((void *) p, sz);
    }
    for (int i = 0; i < nlibs; i++) {
        size_t sz;
        ...
        if (!p)
            continue;
        struct symc_provider_ctx c = { &m, i };
        elf64_foreach_dynsym(p, sz, symc_record_provider, &c);
        munmap((void *) p, sz);
    }
    int total_added = 0;
    for (int i = 0; i < nlibs; i++) {
        ...
        if (!p)
            continue;
        struct symc_edge_ctx c = { &m, i, 0 };
        elf64_foreach_dynsym(p, sz, symc_add_edge, &c);
        munmap((void *) p, sz);
        total_added += c.added;
    }
    symmap_free(&m);
...
static char **build_exec_env(const exec_env_cfg_t *cfg) {
    ...
    /* __r_NP is a user policy knob, not an internal fd handoff: honor
     * an inherited __r_NP=1 from the launch environment in addition to
     * the built-in blacklist, so natural-load can be enabled
     * suite-wide via a single global env var instead of editing
     * exe_is_no_preload_blacklisted per exe.  The inherited copy is
     * still scrubbed by env_is_antirev_managed; a fresh canonical
     * __r_NP=1 is re-injected below, so it also propagates to any
     * protected children (desired — same implicit-dep class).
     * Truthiness matches dlopen_shim's init_shim: set, non-empty,
     * and not "0". */
    const char *np_eq = OBFSTR("__r_NP=");
    const char *np_in = find_env_value(cfg->envp, envc, np_eq, strlen(np_eq));
    int env_no_preload = (np_in && *np_in && strcmp(np_in, "0") != 0);
    int force_no_preload = exe_is_no_preload_blacklisted(base) || env_no_preload;
...
static void build_and_log_deps_graph(int *lib_fds, const char (*lib_names)[MAX_NAME + 1], int nlibs) {
    ...
    /* Wire implicit inter-lib symbol deps into the same adjacency so
     * OP_GET_CLOSURE pulls symbol-providers along.  This is what lets
     * a lib like libdopra (references ICS_RCM_New with no DT_NEEDED
     * edge to libics) load under the default preload path without
     * __r_NP — and keeps preload on so protobuf ODR dedup still works. */
    int sym_edges = augment_closure_with_symbols(lib_fds, lib_names, nlibs);
    ...
    LOG_INFO(
            ...
            "%d libs with 0 deps (+%d symbol-completion edges)\n",
            nlibs, total_edges, zero, sym_edges);
