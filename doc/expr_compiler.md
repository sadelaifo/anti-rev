...
- **`nt` / `pt` 后缀简写**自动展开：变量名以 `nt` 结尾的自动等价于 `<前缀> + <前缀>n`，以 `pt` 结尾的等价于 `<前缀> + <前缀>p`。例如 `aaxnt = aax + aaxn`，`aa1pt = aa1 + aa1p`。领域约定，简化"总量 = 基础值 + 增量"形态。在常量 / 子表达式替换之后、参数检测之前应用 — 函数签名直接是展开后的变量集（`aaxnt + aa1pt` 编出来的函数签名是 `f(aa1, aa1p, aax, aaxn)`）。
