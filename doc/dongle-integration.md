...
### 1.3.1 关键澄清：K_master 在哪里存在？

> ⚠️ **本节是常见误解，务必读懂**

**K_master 这 32 字节明文，永远只以"硅片内电信号"形式存在于加密狗内部，从不以"字节"形式出现在任何软件 / 磁盘文件 / 内存 / 网络 / 日志**。
...
```
软件能做的:
  wrap(K_file_i)          → 狗内部 E(K_master, K_file_i) → 返回 wrapped_dek
  unwrap(wrapped_dek)     → 狗内部 D(K_master, wrapped_dek) → 返回 K_file_i
  sign(challenge)         → 狗内部 ECDSA(K_priv, challenge) → 返回 signature

软件永远不能做的:
  read K_master           ← 没有这种 API
  copy K_master to disk   ← 不可能
  K_master 字节字面上出现在源码 / 配置 / 内存
```

所以本文中**"用 K_master 加密 X"是简写**，严谨说法是 **"调一支持有 K_master 的狗、让狗内部用 K_master 计算 X 的密文"**。两边软件操作完全相同：输入 X，输出密文。区别仅在于 K_master 字节存放的位置 —— **永远是硅片，永远不是软件**。
...
| K_master | 主密钥，**只以电信号形式存在于加密狗硅片内**，永不以字节形式出现在软件 / 磁盘 / 内存 / 网络 |
