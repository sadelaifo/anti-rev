...
{
    /*
     * SVE register names are fixed by architecture (z0..z31, p0..p15), so
     * use static string tables instead of g_strdup_printf'ing them on every
     * CPU realize. The names are stored as borrowed pointers in
     * builder->regs[]/feature->regs[] and consumed read-only by the gdbstub;
     * static const strings are safe and avoid a per-realize leak (the regs[]
     * container is freed by arm_cpu_finalizefn, but its inner string slots
     * are not, so anything heap-allocated for those slots leaks).
     */
    static const char * const sve_z_names[32] = {
        "z0",  "z1",  "z2",  "z3",  "z4",  "z5",  "z6",  "z7",
        "z8",  "z9",  "z10", "z11", "z12", "z13", "z14", "z15",
        "z16", "z17", "z18", "z19", "z20", "z21", "z22", "z23",
        "z24", "z25", "z26", "z27", "z28", "z29", "z30", "z31",
    };
    static const char * const sve_p_names[16] = {
    ...
    for (i = 0; i < 32; i++) {
        gdb_feature_builder_append_reg(&builder, sve_z_names[i], reg_width,
                                       reg++, "svev", NULL);
    }
    ...
    for (i = 0; i < 16; i++) {
        gdb_feature_builder_append_reg(&builder, sve_p_names[i], pred_width,
                                       reg++, "svep", NULL);
    }
