...
static void arm_cpu_finalizefn(Object *obj)
{
    ...
    /*
     * The following per-CPU allocations are made during arm_cpu_realizefn
     * but were not freed anywhere. On qemu-user, where each guest
     * pthread_create creates+destroys a CPU object, leaking these caused
     * unbounded RSS growth (~30+ KB per pthread cycle).
     *
     * cpreg_indexes/cpreg_values come from arm_init_cpreg_list().
     * (cpreg_vmstate_* are NULL in user mode; they're populated only by
     * cpu_pre_save/cpu_post_load and are freed in cpu_post_save.)
     *
     * dyn_*_feature.desc.{xml,regs} come from gdb_feature_builder_end():
     *   xml is g_strjoinv'd; regs is g_ptr_array_free(..., FALSE)'d.
     * dyn_sysreg_feature.data.cpregs.keys comes from arm_gen_dynamic_sysreg_feature.
     * The individual register name strings inside regs[] are mixed:
     *   for sysreg they are pointers into ARMCPRegInfo entries owned by
     *   cp_regs (already freed by g_hash_table_destroy above), so we must
     *   not free the inner strings; just the outer array container.
     */
    g_free(cpu->cpreg_indexes);
    g_free(cpu->cpreg_values);
    g_free((char *)cpu->dyn_sysreg_feature.desc.xml);
    g_free((void *)cpu->dyn_sysreg_feature.desc.regs);
    g_free(cpu->dyn_sysreg_feature.data.cpregs.keys);
    g_free((char *)cpu->dyn_svereg_feature.desc.xml);
    g_free((void *)cpu->dyn_svereg_feature.desc.regs);
    g_free((char *)cpu->dyn_smereg_feature.desc.xml);
    g_free((void *)cpu->dyn_smereg_feature.desc.regs);
    g_free((char *)cpu->dyn_m_systemreg_feature.desc.xml);
    g_free((void *)cpu->dyn_m_systemreg_feature.desc.regs);
#ifndef CONFIG_USER_ONLY
    g_free((char *)cpu->dyn_m_secextreg_feature.desc.xml);
    g_free((void *)cpu->dyn_m_secextreg_feature.desc.regs);
#endif
#ifdef CONFIG_USER_ONLY
    g_free((char *)cpu->dyn_tls_feature.desc.xml);
    g_free((void *)cpu->dyn_tls_feature.desc.regs);
#endif
