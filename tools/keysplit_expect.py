...
the stub's `keysplit[dbg]` log lines, to localize a "decryption failed"
mismatch:
...
Output field -> stub log line:
  ...
  version_field_hash     -> keysplit[dbg]: ... field(len=N) hash=
...
def main() -> None:
    ...
    print(f"version_field         : {version_bytes!r} (len {len(version_bytes)})")
