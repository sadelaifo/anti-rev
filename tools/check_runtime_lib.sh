...
#
# When set up correctly the linker should find the lib via
...
#
# Runs the protected bin in background just long enough to capture the
# symlink dir, then kills it.  Stdout/stderr of the bin go to
# /tmp/run.log for inspection.
...
if [[ -z $DIR ]]; then
    echo "no symlink dir found for pid $PID — stub didn't create one" >&2
...
# Build the list of libs to inspect.
if [[ -n $LIBNAME ]]; then
    libs=("$DIR/$LIBNAME")
else
    mapfile -t libs < <(find "$DIR" -maxdepth 1 -type l)
fi
