...
- **antirev_patch** (`stub/patch_shim.c`): a separate, keyless `LD_PRELOAD` shim (`antirev_patch_<arch>.so`) for **hot-patch / live-patch tooling**. Loaded into an *unprotected* injector process; intercepts `open`/`open64`/`openat`/`openat64` of encrypted `.patch` files and returns the daemon-decrypted memfd so the injector reads plaintext transparently. Fully self-contained: no key, no crypto, no `daemon_client.c` — a pure socket client that finds the daemon via the discovery file (see "Hot-patch support" below). Deliberately NOT part of `antirev_shim`; its `open`/`openat` interceptors are independent of `aarch64_extend_shim`'s `.elf` `openat` redirect and are only ever loaded into the injector, never a protected process.
...
- **Hot-patch support (`antirev_patch.so` + `OP_GET_PATCH`)**: lets a business live-patch tool upgrade a running process without antirev coupling. Flow: (1) the operator drops an *encrypted* `.patch` file (raw `encrypt-lib` output — `ANTREV01` + iv + tag + ct, basename preserved) into the daemon's scan dir; (2) the injector is launched with `LD_PRELOAD=antirev_patch_<arch>.so`; (3) when the injector `open()`s the `.patch` path, the shim discovers the daemon and sends `OP_GET_PATCH(basename)`; (4) the daemon resolves the basename under its scan dir, **re-reads the AES key from its own `/proc/self/exe` trailer for this one decrypt** (the startup key is wiped after `bind`; lazy decrypt re-reads then `explicit_bzero`s, keeping key residency minimal), decrypts into a **fresh memfd per request** (independent file offset per client), replies with `OP_LIB` + fd, and closes its own copy; (5) the shim returns that fd to the injector, which reads plaintext. Design rationale (see chat history / decoupling discussion): the *business injector stays antirev-blind* — it calls plain `open()`; all antirev knowledge lives in the antirev-owned shim + daemon. There is **no `reload()` API** — patches are decrypted lazily on first `open()`, so the business/orchestration side never calls an antirev API; the contract is just "drop an encrypted file in a directory." `.patch` files are **not** scanned at daemon startup (`collect_enc_paths` only queues `.so`/`.elf`), so they cost nothing until requested.
    - **Discovery file**: a keyless injector can't re-derive the key-hashed abstract socket name, so the daemon publishes its (non-secret) abstract socket name to `<scan_dir>/.antirev-libd.sock` at startup (before the daemonising fork; unlinked on clean shutdown). The shim walks up from the `.patch` path's directory to find it, reads the name, and connects. The filename is a plaintext literal in both `stub.c` and `patch_shim.c` — it sits next to the `.antirev-libd` binary, which is already visible on disk, so it leaks nothing new (could be folded into `gen_obf.py` later if the shim takes on `obf.h`).
    - **Traversal guard**: `OP_GET_PATCH` rejects names containing `/` — only basenames resolved under the daemon's own scan dir are served, never client-supplied paths.
    ...
    - **Env**: `ANTIREV_PATCH_LOG=<path>` — line-buffered log of every `open` decision, discovery lookup, and daemon fetch in `antirev_patch.so`.
