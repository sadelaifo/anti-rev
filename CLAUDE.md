...
- **dlopen_shim**: LD_PRELOAD shim that redirects `dlopen()` calls to decrypted memfd-backed libraries. Two resolution modes: lazy fetch via `ANTIREV_LIBD_SOCK` + `ANTIREV_FD_MAP_LIBS` (names only — each lib is fetched from the daemon on first use), or eager `ANTIREV_FD_MAP` (preallocated fds, Mode A bundled libs).
...
- The daemon mode splits libs into DT_NEEDED (resolved via symlink dir on `LD_LIBRARY_PATH`) and dlopen'd (lazy-fetched by name through the kept-open daemon socket). DT_NEEDED libs are NOT on `LD_PRELOAD` — glibc's normal BFS resolves them through the symlink dir, preserving the original symbol lookup order. `LD_PRELOAD` contains only exe_shim + dlopen_shim.
- **Daemon protocol v2** (`stub/stub.c`): length-prefixed binary messages with opcodes `OP_INIT` / `OP_GET_LIB` / `OP_LIST` / `OP_BYE` (client→daemon) and `OP_BATCH` / `OP_END` / `OP_LIB` / `OP_NAMES` (daemon→client). A single-threaded epoll loop serves clients, keeps connections alive for on-demand `OP_GET_LIB` requests, and uses `SCM_RIGHTS` to pass one memfd per lib on demand. `stub.c` Mode C (daemon client) fetches only the DT_NEEDED subset up front via `OP_INIT` + filter, then hands the socket fd off to the exec'd process via `ANTIREV_LIBD_SOCK`.
- **Lazy fetch in dlopen_shim** (`stub/dlopen_shim.c`): at first `dlopen()` for a name in `ANTIREV_FD_MAP_LIBS`, the shim sends `OP_GET_LIB` on the inherited daemon socket, receives one fd via `SCM_RIGHTS`, and calls the real `dlopen("/proc/self/fd/N")`. The fd is cached and held open for the life of the process — this keeps the `/proc/self/fd/N` path stable so glibc's link-map dedup returns the same handle on subsequent `dlopen()` of the same lib. Closing the fd and letting the kernel reuse the number breaks dedup: two different libs would collide on the same path.
