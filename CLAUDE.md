...
- **antirev_shim**: single LD_PRELOAD shim built from `stub/exe_shim.c` + `stub/dlopen_shim.c` — one `.so` per arch (`antirev_shim_x86_64.so`, `antirev_shim_aarch64.so`). Intercepts `readlink` / `realpath` / `getauxval` to hide memfd paths AND `dlopen()` to redirect encrypted libs to `/proc/self/fd/N` (eager via `ANTIREV_FD_MAP`, lazy via daemon `OP_GET_CLOSURE`). The two source files keep the identity-interception and dlopen-interception concerns separate; the linker combines them into one DSO with no symbol overlap and independent file-scope state + constructors.
- **aarch64_extend_shim** (aarch64-only): separate LD_PRELOAD shim that (a) intercepts `ANTI_LoadProcess(struct ANTI_ProcessInfo *)` to rewrite `info->ltrBin` from an on-disk `.elf` path to a `/proc/self/fd/N` memfd populated from the daemon; (b) hosts the aarch64 `popen`/`pclose` workaround (glibc's vfork-based popen corrupts memfd-heavy parent processes on this arch). Kept as its own DSO so x86 builds don't pull in any of its code.
