...
- `__r_NP=1` — disables the per-dep preload loop entirely. `fetch_closure` still fetches the closure and materializes symlinks, but the caller's `real_dlopen(root)` then triggers glibc's normal recursive DT_NEEDED walk, which maps every dep first and runs all ctors together at the end — matching plaintext semantics exactly. This is the robust fix for **implicit inter-lib symbol dependencies**, where one business lib references a symbol provided by a sibling lib without an explicit `DT_NEEDED` edge (e.g. `_ZN3BBBC1EPN6google8protobuf5ArenaE` — `BBB::BBB(google::protobuf::Arena*)` — needed by libccc but provided by libbbb with no edge between them). The per-dep preload loop loads ctors in isolation and can trip lazy-binding failures across the implicit boundary; natural load sidesteps it. The daemon cannot statically infer these edges in general — when the provider is the protected exe itself or a plaintext third-party lib (the **Class-3** case, the dominant one in practice), only natural load resolves it — so `__r_NP` is the robust answer rather than any closure-graph heuristic. **`__r_NP` mode forces `RTLD_GLOBAL` onto the root `real_dlopen`** (`dlopen_shim.c`): glibc propagates `RTLD_GLOBAL` to the entire freshly-loaded dependency subtree, so DSOs that statically link the same `.pb.o` interpose their `descriptor_table_<file>_2eproto` first-definition-wins instead of each re-registering — i.e. `__r_NP` gives plaintext-equivalent implicit-dep resolution **and** protobuf ODR dedup in one shot, with no per-binary patchelf. Without this the caller's flags (typically `RTLD_LOCAL`) would leave each dep in a private scope and libprotobuf would abort with "File already exists in database" (regression covered by `test_dlopen_interpose_np`, which runs the `test_dlopen_interpose` scenario under `__r_NP=1`). **Enablement**: `build_exec_env` (`stub.c`) sets `__r_NP=1` for the protected exe if the exe basename is in `exe_is_no_preload_blacklisted` **or** an `__r_NP=1` (non-empty, not `"0"`) was present in the launch environment. The stub still scrubs the *inherited* copy via `env_is_antirev_managed` and re-injects a fresh canonical one, so a single global `export __r_NP=1` enables natural-load suite-wide (and propagates to protected children) without editing the C blacklist per exe. `__r_NP=0`/unset leaves the blacklist as the only trigger. Covered by `test_env_no_preload` (inherited `__r_NP=1` honored; also a Class-3 plaintext-provider implicit dep that only natural load can resolve). Remaining tradeoff: without preload, any lib whose old-style `DT_RPATH` points at the encrypted on-disk dir will have its own DT_NEEDED search hit ciphertext before `LD_LIBRARY_PATH` catches it (libs that emit `DT_RUNPATH`, searched after `LD_LIBRARY_PATH`, are unaffected) — so `__r_NP` is opt-in, not the hardcoded default. Adding the missing `DT_NEEDED` edge to the business build (`target_link_libraries(CCC PRIVATE BBB)`) is still the cleanest per-edge fix where feasible since it works for every loader, but is no longer required to keep protobuf working under `__r_NP`.
