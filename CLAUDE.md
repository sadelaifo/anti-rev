...
- **Module sources** (`kmod2/module/`): `super.c` (fs registration, `mount_nodev`, option parse, key acquisition, inode/sb lifecycle), `inode.c` (lookup proxied to lower tree, `iget5_locked` caching keyed on lower dentry so one antirevfs inode == one shared page cache per path, getattr reporting plaintext size), `file.c` (`read_folio` decrypt-on-fault, dir `iterate_shared`, generic file ops + `generic_file_mmap`), `crypto.c` (`gcm(aes)` via kernel crypto API; file layout has tag *before* ct, so it builds a `[ct||tag]` buffer for the AEAD), `antirevfs.h`.
- **Strict mode + passthrough**: a file under the mount lacking `ANTREV01` magic returns `-EIO` unless its extension is in the mount's `passthrough=` whitelist (colon-separated, since `,` separates mount options). Decided at inode-classify time (open of lower file to sniff magic).
...
- **Kernel compat**: `compat.h` + inline `LINUX_VERSION_CODE` guards span mainline 6.8 down to **SLES 12 / 4.12.14-default** (the real x86_64 deployment target). Guarded APIs: `read_folio`↔`readpage` (5.18), crypto async-wait trio `DECLARE_CRYPTO_WAIT`/`crypto_req_done`/`crypto_wait_req` (4.16; hand-rolled completion below that), `getattr`/`generic_fillattr` signature bands (5.12/6.3/6.6), `alloc_inode_sb` (5.17), `free_inode`↔`destroy_inode`+`call_rcu` (5.2), `kernel_read` signature (4.14, via `arev_kernel_read`), `SB_*`↔`MS_*` flags (4.15), `hex.h` split (5.18). Enterprise kernels backport APIs while keeping an old version code — override macros like `-DAREV_NEW_KERNEL_READ` handle the mismatch. **`-fpatchable`-style caveat**: only the 6.8 path is build+runtime verified here; the 4.12 path is build-tested on the SLES box (I can't compile 4.12 on the 6.8 dev box).
- **Build/test**: compile with the kernel's own compiler major version — `make -C kmod2/module CC=gcc-12` on the 6.8 dev box, plain `make` (after `zypper install kernel-default-devel`) on SLES. End-to-end: `sudo bash kmod2/tests/test_antirevfs.sh` (9 checks: decrypt correctness, ciphertext-on-disk, dlopen through mount, real path / no memfd in maps, strict-mode reject, passthrough, wrong-key rejection, cross-process PFN sharing). Validated on kernel 6.8.0-117. Resolves DESIGN.md open question #8.
- **Not yet done**: module signing for Secure Boot, key rotation, DKMS `.deb`/`.rpm` packaging.
