...
6. **Strict-mode default**: files under an antirevfs mount must carry `ANTREV01` magic; missing-magic files return `-EIO`. Mixed-content directories (e.g., `lib/module_xx/data.json` alongside `lib/module_xx/foo.so`) handled via an explicit extension passthrough whitelist at mount time, or — for a *complete* mixed read-only tree (real `lib/`/`bin/` with encrypted ELFs + `.py`/`.pyc`/`.sh`/`.txt` data + third-party plaintext libs) — the `passdata` mount option, which serves any non-`ANTREV01` file as plaintext passthrough. The packer (`mirror_plaintext`, default on) mirrors those plaintext files verbatim into the tree so they are visible. The mount is read-only by design: edits to `bin/`/`lib/` are intentionally impossible, and runtime-written files (python `.txt`, `__pycache__`) must be redirected to a writable path outside the mount (an overlayfs upper, or the app's own writable dir).
...
- Resolved mixed-content (`.so` + `.py` + `.json` in same dir) handling: strict-mode + extension passthrough whitelist + encrypt `.py` files alongside `.so`. Extended for *complete read-only* `bin/`/`lib/` trees: the packer's `mirror_plaintext` mode mirrors all non-ELF data files (and blacklisted third-party ELFs) verbatim into the tree, and the `passdata` mount option serves every non-`ANTREV01` file as plaintext passthrough. Edits to those dirs are intentionally disallowed (read-only mount); config and editable scripts live outside the encrypted set (plaintext on the real fs), and protected Python stays compiled (`.so`) rather than shipped as editable source. Runtime writes go to a writable layer outside the mount (overlayfs upper).
