...
# round-trip: decrypt with the master key via a tiny python check.
# key-in-.ko: the container is KEYLESS (MAGIC|iv|tag|ct, no trailer) — the key
# lives in the .ko, not the file — so we decrypt with the packer key directly.
...
assert blob[:8] == bytes.fromhex("a74c2e91d63b085f"), "missing header magic"
assert len(blob) == 36 + len(open(plain, "rb").read()), "container should be keyless (hdr+ct, no key trailer)"
iv, tag, ct = blob[8:20], blob[20:36], blob[36:]
dec = AESGCM(key).decrypt(iv, ct + tag, None)
sys.exit(0 if dec == open(plain, "rb").read() else 1)
PY
[[ $? -eq 0 ]] && ok "decrypt round-trips (keyless container, key from .ko)" || bad "decrypt mismatch"
