...
connection_recv(UA_Connection *connection, UA_ByteString *response,
     ...
     * Use poll() rather than select() to support fd numbers >= FD_SETSIZE. */
    struct pollfd recvPoll;
    recvPoll.fd = (int)connection->sockfd;
    recvPoll.events = POLLIN;
    recvPoll.revents = 0;
    int resultsize = UA_poll(&recvPoll, 1, (int)timeout);
    ...
    if(resultsize == -1) {
        /* The call to poll was interrupted. Act as if it timed out. */
...
ServerNetworkLayerTCP_listen(UA_ServerNetworkLayer *nl, UA_Server *server,
    ...
    /* Build a pollfd array: listen sockets followed by established connections.
     * Using poll() rather than select() avoids the FD_SETSIZE (1024) limit. */
    size_t pollMax = (size_t)layer->serverSocketsSize + (size_t)layer->connectionsSize;
    if(pollMax == 0)
        return UA_STATUSCODE_GOOD;

    struct pollfd *pfds = (struct pollfd*)UA_calloc(pollMax, sizeof(struct pollfd));
    if(!pfds)
        return UA_STATUSCODE_GOOD;

    for(UA_UInt16 i = 0; i < layer->serverSocketsSize; i++) {
        pfds[i].fd = (int)layer->serverSockets[i];
        pfds[i].events = POLLIN;
    }
    size_t connIdx = layer->serverSocketsSize;
    {
        ConnectionEntry *ce;
        LIST_FOREACH(ce, &layer->connections, pointers) {
            if(connIdx >= pollMax) break;
            pfds[connIdx].fd = (int)ce->connection.sockfd;
            pfds[connIdx].events = POLLIN;
            connIdx++;
        }
    }

    if(UA_poll(pfds, (nfds_t)connIdx, (int)timeout) < 0) {
        UA_LOG_SOCKET_ERRNO_WRAP(
            UA_LOG_DEBUG(layer->logger, UA_LOGCATEGORY_NETWORK,
                           "Socket poll failed with %s", errno_str));
        UA_free(pfds);
        return UA_STATUSCODE_GOOD;
    }
    ...
    LIST_FOREACH_SAFE(e, &layer->connections, pointers, e_tmp) {
        ...
        /* Find this connection's slot in the poll array */
        short crevents = 0;
        for(size_t k = layer->serverSocketsSize; k < connIdx; k++) {
            if(pfds[k].fd == (int)e->connection.sockfd) {
                crevents = pfds[k].revents;
                break;
            }
        }
        if(!(crevents & (POLLIN | POLLERR | POLLHUP | POLLNVAL)))
            continue;
...
UA_ClientConnectionTCP_poll(UA_Connection *connection, UA_UInt32 timeout,
...
#else
    /* Wait in a poll-call until the connection fully opens or the timeout
     * happens. poll() avoids the FD_SETSIZE (1024) limit of select(). */
    struct pollfd connPoll;
    connPoll.fd = (int)connection->sockfd;
    connPoll.events = POLLOUT;
    connPoll.revents = 0;
    int pollTimeoutMs = (int)(timeout_usec / 1000);

    int ret = UA_poll(&connPoll, 1, pollTimeoutMs);

    // When poll fails abort connection
    if(ret == -1) {
        UA_LOG_WARNING(logger, UA_LOGCATEGORY_NETWORK,
                       "Connection to %.*s failed with error: %s",
                       (int)tcpConnection->endpointUrl.length,
    ...
    int resultsize = (connPoll.revents & POLLOUT) ? 1 : 0;
#endif
